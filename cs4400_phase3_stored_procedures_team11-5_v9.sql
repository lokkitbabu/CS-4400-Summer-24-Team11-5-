...
SELECT 
    departure_port.portID as departing_from,
    arrival_port.portID as arriving_at,
    COUNT(distinct c.cruiseID) as num_cruises,
    GROUP_CONCAT(distinct c.cruiseID order by c.cruiseID SEPARATOR ', ') as cruise_list,
    MIN(c.next_time) as earliest_arrival,
    MAX(c.next_time) as latest_arrival,
    GROUP_CONCAT(distinct s.locationID order by s.locationID SEPARATOR ', ') as ship_list
from 
    cruise c
join 
    route_path rp ON c.routeID = rp.routeID AND c.progress = rp.sequence
join 
    leg l ON rp.legID = l.legID
join 
    ship_port departure_port ON l.departure = departure_port.portID
join 
    ship_port arrival_port ON l.arrival = arrival_port.portID
join 
    ship s ON c.support_cruiseline = s.cruiselineID AND c.support_ship_name = s.ship_name
where 
    c.ship_status = 'sailing'
group by 
...
create function status_check (ip_cruiseID varchar (50))
returns int
Deterministic
Begin
	declare v_progress int;
    
    select progress into v_progress from cruise where cruiseID = ip_cruiseID;
...
select
    ...
    COUNT(DISTINCT c.cruiseID) AS num_cruises,
    GROUP_CONCAT(DISTINCT c.cruiseID ORDER BY c.cruiseID SEPARATOR ', ') AS cruise_list,
    MIN(c.next_time) AS earliest_departure,
    MAX(c.next_time) AS latest_departure,
    GROUP_CONCAT(DISTINCT s.locationID ORDER BY s.locationID SEPARATOR ', ') AS ship_list
FROM 
    cruise c
JOIN 
    route_path rp ON c.routeID = rp.routeID AND rp.sequence = (progress+1)
JOIN 
    leg l ON rp.legID = l.legID
JOIN 
    ship_port sp ON l.departure = sp.portID
JOIN 
    ship s ON c.support_cruiseline = s.cruiselineID AND c.support_ship_name = s.ship_name
WHERE 
    c.ship_status = 'docked'
    -- AND EXISTS (
       --  SELECT 1
       --  FROM route_path rp2
       --  WHERE rp2.routeID = c.routeID AND rp2.sequence > (select status_check(c.cruiseID))
    -- )
GROUP BY 
    sp.portID;
...
select     
	departure_port.portID AS departing_from,
    arrival_port.portID AS arriving_at,
    COUNT(DISTINCT s.locationID) AS num_ships,
    GROUP_CONCAT(DISTINCT s.locationID ORDER BY s.locationID SEPARATOR ', ') AS ship_list,
    GROUP_CONCAT(DISTINCT c.cruiseID ORDER BY c.cruiseID SEPARATOR ', ') AS cruise_list,
    MIN(c.next_time) AS earliest_arrival,
    MAX(c.next_time) AS latest_arrival,
    SUM(CASE WHEN cr.personID IS NOT NULL THEN 1 ELSE 0 END) AS num_crew,
    SUM(CASE WHEN p.personID IS NOT NULL THEN 1 ELSE 0 END) AS num_passengers,
    COUNT(DISTINCT po.personID) AS num_people,
    GROUP_CONCAT(DISTINCT po.personID ORDER BY CAST(SUBSTRING(po.personID, 2) AS UNSIGNED) SEPARATOR ', ') AS person_list
FROM 
    cruise c
JOIN 
    route_path rp ON c.routeID = rp.routeID AND c.progress = rp.sequence
...
JOIN 
    ship s ON c.support_cruiseline = s.cruiselineID AND c.support_ship_name = s.ship_name
JOIN 
    person_occupies po ON s.locationID = po.locationID
LEFT JOIN 
    crew cr ON po.personID = cr.personID
LEFT JOIN 
    passenger p ON po.personID = p.personID
WHERE 
    c.ship_status = 'sailing'
GROUP BY 
    departing_from, arriving_at;
...
select
	departure_port.portID AS departing_from,
    departure_port.locationID AS ship_port,
    departure_port.port_name as port_name,
    departure_port.city as city,
    departure_port.state as state,
    departure_port.country as country,
    SUM(CASE WHEN cr.personID IS NOT NULL THEN 1 ELSE 0 END) AS num_crew,
    SUM(CASE WHEN p.personID IS NOT NULL THEN 1 ELSE 0 END) AS num_passengers,
    COUNT(DISTINCT po.personID) AS num_people,
    GROUP_CONCAT(DISTINCT po.personID ORDER BY CAST(SUBSTRING(po.personID, 2) AS UNSIGNED) SEPARATOR ', ') AS person_list
...
FROM
    cruise c
JOIN 
    route_path rp ON c.routeID = rp.routeID AND c.progress+1 = rp.sequence
JOIN 
    leg l ON rp.legID = l.legID
JOIN 
    ship_port departure_port ON l.departure = departure_port.portID
JOIN 
    ship s ON c.support_cruiseline = s.cruiselineID AND c.support_ship_name = s.ship_name
JOIN 
    person_occupies po ON (departure_port.locationID = po.locationID and po.personID != 'p0')
LEFT JOIN 
    crew cr ON (po.personID = cr.personID )
LEFT JOIN 
    passenger p ON po.personID = p.personID
...
select     
    ...
    (SELECT 
        GROUP_CONCAT(
            CASE 
                WHEN rp_inner.sequence = 1 THEN CONCAT(l_inner.departure, '->', l_inner.arrival)
                ELSE CONCAT(',', l_inner.departure, '->', l_inner.arrival)
            END
            ORDER BY rp_inner.sequence
            SEPARATOR ''
        )
     FROM route_path rp_inner
     JOIN leg l_inner ON rp_inner.legID = l_inner.legID
     WHERE rp_inner.routeID = r.routeID
    ) AS port_sequence
FROM 
    route r
LEFT JOIN 
...
LEFT JOIN 
    leg l ON rp.legID = l.legID
LEFT JOIN 
    cruise c ON r.routeID = c.routeID
GROUP BY 
    r.routeID;
